...
- **창호**: Spring Security와 JWT를 활용하여 회원 기능과 팔로우 기능을 담당하였습니다. 이전의 프로젝트에서 JWT를 활용해본 경험은 있었으나 Spring Security가 처음이어서 많은 어려움이 있었습니다. Security의 동작 과정을 이해하면서 로직을 짜는 과정이 어려웠고, JWT를 이용한 Access Token과 Refresh Token의 용도에 대해 생각하고 알맞은 역할을 부여하는 데 많은 시간을 투자했습니다. 또한 로그아웃 기능을 구현하면서 Redis와 Refresh Token을 이용하여 로직을 작성하는 과정에서 Redis에 Acess Token을 저장해야 할지, Refresh Token을 저장해야 할지에 대해서도 많은 고민을 할 수 있었습니다. 이번 프로젝트를 통해서 새로운 기술을 습득할 때 공식문서의 중요성을 깨달았고, 고민 과정을 통해 Spring의 동작 과정, 인증 및 인가와 같은 개념들에 대해 학습할 수 있어서 뜻깊은 시간이었습니다.
...
- **창호**:
1. 2주라는 짧은 기간동안 진행했기에 개발을 진행하면서 부실했던 설계의 문제점을 느낄 수 있었습니다. 팀 프로젝트를 진행할 때, 탄탄한 설계가 바탕이 된다면 개발의 속도 또한 더 빨라질 수 있었을거라 생각하며 다음에 팀 프로젝트를 한다면 이 부분을 개선하고 싶습니다.
2. Authorization을 적용하였으나, 제대로 활용하지 못하였습니다. USER, ADMIN의 2가지 권한을 만들었는데 실질적으로 사용한 것은 로그인 여부에 따라 USER라면 접근할 수 있는 인가 설정만 적용했습니다. 다음 프로젝틀르 진행한다면 Authorization을 폭넓게 활용해보고 싶습니다.
- **서현**: [아쉬운 점 및 개선 사항]
